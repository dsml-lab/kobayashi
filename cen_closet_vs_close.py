...
from datasets import load_datasets, NoisyDataset
from models import load_models
from viz_colored_alpha_gif_csv_miseruyou import target_plot_probabilities
from viz_colored_alpha_seed_gattai import evaluate_label_changes
from train import (
...
def load_training_data(args):
    """
    データセットの読み込み・前処理を効率的に行う関数。
    保存済みデータを使う場合と、オリジナルデータからノイズ付与する場合に対応。
    
    Returns:
        tuple: (train_dataset, test_dataset, imagesize, num_classes, in_channels,
                num_digits, num_colors, noisy_targets, noise_info)
    """
    import os
    import numpy as np
    import torch
    ...
    # まずは共通のデータセットロード（保存済みデータの有無に関わらず）
    train_dataset, test_dataset, imagesize, num_classes, in_channels = load_datasets(
        args.dataset, args.target, args.gray_scale, args
    )

    # クラス数の設定（targetが'combined'ならdigit, colorそれぞれ10クラス）
    if args.target == 'combined':
        num_digits, num_colors = 10, 10
    else:
        num_digits, num_colors = 10, 1

    # 保存済みデータを利用する場合
    if args.use_saved_data:
        print(f'Loading pre-saved : {args.use_saved_data}')
        ...
        # np.loadにmmap_modeを指定してメモリ効率を改善（必要に応じて）
        x_train = torch.from_numpy(np.load(os.path.join(data_dir, f"noisy{args.label_noise_rate}_x_train_colored.npy"), mmap_mode='r'))
        y_train_noisy = torch.from_numpy(np.load(os.path.join(data_dir, f"noisy{args.label_noise_rate}_y_train_colored.npy"), mmap_mode='r'))
        noise_info = torch.from_numpy(np.load(os.path.join(data_dir, f"noisy{args.label_noise_rate}_info_y_train_colored.npy"), mmap_mode='r'))
        
        train_dataset = TensorDataset(x_train, y_train_noisy)
        train_dataset = NoisyDataset(train_dataset, noise_info)
        # noisy_targetsはTensorDatasetの場合はtensors[1]がそのまま利用できる
        noisy_targets = y_train_noisy if hasattr(train_dataset.dataset, 'tensors') else torch.tensor(train_dataset.dataset.targets)
    ...
    return train_dataset, test_dataset, imagesize, num_classes, in_channels, num_digits, num_colors, noisy_targets, noise_info
def main():
    print('Start session')
    ...
    try:
        ...
        try:
            x_noisy, idx_noisy, distance_noisy = find_closest_data_point_to_centroid(
                centroid1,
                train_dataset.dataset,
                original_targets,
                class_label=n1,
            ...
            print(f"Found noisy data point idx={idx_noisy} with distance {distance_noisy:.4f}")

            # Find closest clean data point to x_noisy (y) using select_n2
            y_clean, idx_clean, distance_clean = select_n2(
                n1=x_noisy,  # 【変更】：x_noisy を n1 として渡す
                idx1=idx_noisy,  # 【変更】：x_noisy のインデックス
                target="combined",  # 【変更】：ターゲットを 'combined' に設定
                mode=mode2,  # 【変更】：'no_noise' モードで選択
            ...
            if y_clean is None:
                print("No suitable clean data point y found.")
                return
            ...
            base_alpha_csv_dir = f"./alpha_test/seed/shufle/lr_0.0/ori_closet_{experiment_name}/{mode1}_{mode2}/{label_y_clean}/{label_x_noisy}"
            # ユニークなディレクトリパスを取得
        ...
        axes[1].set_title(
            f"Noisy Data (idx={idx_noisy})\nOri: {orig_label_x_noisy}, Noisy: {noisy_label_x_noisy}\nDistance to Centroid: {distance_noisy:.2f}\nDistance to x y: {distance_x_y:.2f}"
        )
        axes[1].axis('off')
        ...
        for epoch in range(0, args.epoch + 1):
            checkpoint_path = f"save_model/Colored_EMSNIT/noise_rate=0_sigma=1000/{experiment_name}/model_epoch_{epoch}.pth"
            ...
            alpha_csv_path, raw_probs_csv_path = setup_alpha_csv_logging_save_dir(save_dir, epoch, n1, n2, mode1, mode2)
            log_alpha_test_results_save(alpha_logs, alpha_csv_path, raw_probs_csv_path)
    except Exception as e:
    ...
    target = ["combined"]
    for tg in target:
        #evaluate_label_changes(directory=alpha_csv_dir, mode="alpha", target=tg, y_lim=(0,0.03), smoothing=True, smoothing_window=5)
        evaluate_label_changes(directory=alpha_csv_dir, mode="alpha", target=tg, y_lim=(0,0.03), smoothing=False)
        evaluate_label_changes(directory=alpha_csv_dir, mode="epoch", target=tg, y_lim=(0,0.4), smoothing=False)
    ...
    target_plot_probabilities(alpha_csv_dir, targets="combined", gif=True,show_legend=False, gif_output=f"combined.gif", epoch_start=0, epoch_end=150, epoch_step=2)
    clear_memory()
