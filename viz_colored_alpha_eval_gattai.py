...
def create_combined_figure(root_path, fig_dir_name='fig_and_log', 
    ...
    :param root_path: ルートディレクトリのパス（例: .../no_noise_no_noise/0）
    :param fig_dir_name: 図とログが格納されているサブディレクトリ名
    :param smoothing: Trueの場合、スムージングされた画像を使用。Falseの場合、元の画像を使用。
    :param first_dir: 処理を開始するサブディレクトリ名
    :param end_dir: 処理を終了するサブディレクトリ名
    """
    # ログの設定
    logging.basicConfig(filename='combined_figure.log', level=logging.INFO,
    ...
    # combined_fig_dir を root_path に基づいて設定
    # 例: root_path が 'path/to/no_noise_no_noise/0' の場合、'path/to/no_noise_no_noise/combined_fig' を作成
    parent_dir = os.path.dirname(root_path)  # 'path/to/no_noise_no_noise'
    combined_fig_dir = os.path.join(parent_dir, 'combined_fig')
    os.makedirs(combined_fig_dir, exist_ok=True)

    # サブディレクトリ名のリストを取得
    subdirs = sorted(
        [d for d in os.listdir(root_path) if os.path.isdir(os.path.join(root_path, d))],
        key=lambda x: (0, int(x)) if x.isdigit() else (1, x)  # 修正箇所
    )

    # first_dir と end_dir が指定されている場合、その範囲でサブディレクトリをフィルタリング
    if first_dir and end_dir:
        try:
            first_index = subdirs.index(first_dir)
            end_index = subdirs.index(end_dir)
            if first_index > end_index:
                first_index, end_index = end_index, first_index  # 順序が逆の場合は入れ替え
            subdirs = subdirs[first_index:end_index + 1]
    ...
    for subdir in subdirs:
        fig_and_log_path = os.path.join(root_path, subdir, fig_dir_name)
        if os.path.isdir(fig_and_log_path):
            ...
            warning_msg = f"警告: {fig_dir_name} ディレクトリが {os.path.join(root_path, subdir)} 内に存在しません。"
            print(warning_msg)
            logging.warning(warning_msg)
    ...
    # 出力ファイル名の生成
    output_filename = f"variance_{variance}_label_noise_rate{label_noise_rate}_{first_dir}_{end_dir}.png"
...
if __name__ == "__main__":
    ...
    root_directory = 'alpha_test/test_closet_cnn_5layers_distribution_colored_emnist_variance3162_combined_lr0.01_batch256_epoch2000_LabelNoiseRate0.5_Optimsgd_Momentum0.0/noise_no_noise/1/'

    # fig_dir_name を 'fig_and_log' に設定
    # smoothing=True にするとスムージングされた画像を使用します。必要に応じて変更してください。
    # first_dir と end_dir を指定して範囲を設定します。例えば '1' から '5' まで
    
    create_combined_figure(
        root_path=root_directory, 
        fig_dir_name='fig_and_log', 
        smoothing=False,
        first_dir='30',      # 開始ディレクトリ名
        end_dir='30'         # 終了ディレクトリ名
    )
    ...
    create_combined_figure(
        ...
        fig_dir_name='fig_and_log', 
        smoothing=False,
        first_dir='10',      # 開始ディレクトリ名
        end_dir='50'         # 終了ディレクトリ名
    )
    create_combined_figure(
        root_path=root_directory, 
        fig_dir_name='fig_and_log', 
        smoothing=False,
        first_dir='60',      # 開始ディレクトリ名
        end_dir='80'         # 終了ディレクトリ名
    )
"""
