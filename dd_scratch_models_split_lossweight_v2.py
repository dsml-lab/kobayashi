...
def train_model(model, train_loader, optimizer, criterion_noisy, criterion_clean, weight_noisy, weight_clean, device, num_colors, num_digits):
    model.train()
    running_loss = 0.0
    total_samples = 0
    correct_total = 0
    ...
    for inputs, labels, noise_labels in train_loader:
        ...
        if idx_noisy.sum() > 0:
            ...
            per_sample_loss_noisy = criterion_noisy(outputs_noisy, labels_noisy)
            ...
            digit_predictions_noisy = predicted[idx_noisy] // num_colors
            color_predictions_noisy = predicted[idx_noisy] % num_colors
            ...
            # 損失のリストに追加
            loss_values_noisy.extend(per_sample_loss_noisy_weighted.detach().cpu().numpy())
        ...
        if idx_clean.sum() > 0:
            ...
            digit_predictions_clean = predicted[idx_clean] // num_colors
            color_predictions_clean = predicted[idx_clean] % num_colors
            ...
            # 損失のリストに追加
            loss_values_clean.extend(per_sample_loss_clean_weighted.detach().cpu().numpy())
    ...
    var_loss = np.var(loss_values)

    if total_noisy > 0:
        avg_loss_noisy = np.mean(loss_values_noisy)
        var_loss_noisy = np.var(loss_values_noisy)
    else:
        avg_loss_noisy = float('nan')
        var_loss_noisy = float('nan')

    if total_clean > 0:
        avg_loss_clean = np.mean(loss_values_clean)
        var_loss_clean = np.var(loss_values_clean)
    else:
...
def main():
    ...
    for epoch in range(1, args.epoch + 1):
        train_metrics = train_model(
            model, train_loader, optimizer, criterion_noisy, criterion_clean, weight_noisy, weight_clean, device, num_colors, num_digits
        )
