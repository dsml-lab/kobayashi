...
def parse_args():
    ...
    #set dataset setting
    arg_parser.add_argument("-datasets", "--dataset", type=str, choices=["mnist", "emnist", "cifar10", "cifar100", "tinyImageNet", "colored_emnist", "distribution_colored_emnist"], default="cifar10")
...
def add_label_noise(targets, label_noise_rate, num_digits, num_colors):
    """
    Add label noise to the targets ensuring both digit and color labels are different.
    label noise will be both different number and different color.
    
    Args:
        targets (torch.Tensor): The original target labels.
        label_noise_rate (float): The rate of label noise to add.
        num_digits (int): The number of digit classes.
        num_colors (int): The number of color classes.
...
def calculate_accuracy(preds, labels, num_digits, num_colors):
    """
    Calculate accuracy for combined, digit, and color predictions.
    ...
    Returns:
        tuple: Combined accuracy, digit accuracy, and color accuracy.
    """
    # Compute combined accuracy
    combined_correct = (preds == labels).sum().item()
    combined_accuracy = combined_correct / len(labels)
    
    # Split predictions and labels into digit and color components
    preds_digit = preds // num_colors
    preds_color = preds % num_colors
    labels_digit = labels // num_colors
    labels_color = labels % num_colors
    
    # Compute digit accuracy
    digit_correct = (preds_digit == labels_digit).sum().item()
    digit_accuracy = digit_correct / len(labels)
    
    # Compute color accuracy
    color_correct = (preds_color == labels_color).sum().item()
    color_accuracy = color_correct / len(labels)
    
    return combined_accuracy, digit_accuracy, color_accuracy

def train_model(model, train_loader, optimizer, criterion, device, num_digits, num_colors):
    """
    ...
    Returns:
        tuple: A tuple containing the average loss and accuracies on the training dataset.
    """
    model.train()
    running_loss = 0.0
    total_correct_combined = 0
    total_correct_digit = 0
    ...
    for inputs, labels in train_loader:
        ...
        combined_accuracy, digit_accuracy, color_accuracy = calculate_accuracy(predicted, labels, num_digits, num_colors)
        total_correct_combined += combined_accuracy * labels.size(0)
        total_correct_digit += digit_accuracy * labels.size(0)
        total_correct_color += color_accuracy * labels.size(0)
        
    avg_loss = running_loss / len(train_loader)
    combined_accuracy = total_correct_combined / total_samples
    digit_accuracy = total_correct_digit / total_samples
    color_accuracy = total_correct_color / total_samples
    
    return avg_loss, combined_accuracy, digit_accuracy, color_accuracy

def test_model(model, test_loader, criterion, device, num_digits, num_colors):
    """
    ...
    Returns:
        tuple: A tuple containing the average loss, accuracies, and errors on the test dataset.
    """
    model.eval()
    running_loss = 0.0
    total_correct_combined = 0
...
def main():
    ...
    if args.label_noise_rate > 0.0:
        if args.dataset == "colored_emnist" or args.dataset == "distribution_colored_emnist":
            train_dataset.tensors = (train_dataset.tensors[0], add_label_noise(train_dataset.tensors[1], args.label_noise_rate, num_digits, num_colors))
        else:
            train_dataset.targets = add_label_noise(train_dataset.targets, args.label_noise_rate, num_digits, num_colors)
    ...
    if args.dataset == "colored_emnist":
        experiment_name = f'{args.model}_{args.dataset}_{args.target}_lr{args.lr}_batch{args.batch}_epoch{args.epoch}_LabelNoiseRate{args.label_noise_rate}_Optim{args.optimizer}'
    elif args.dataset == "distribution_colored_emnist":
        experiment_name = f'{args.model}_{args.dataset}_Variance{args.variance}_Correlation{args.correlation}_lr{args.lr}_batch{args.batch}_epoch{args.epoch}_LabelNoiseRate{args.label_noise_rate}_Optim{args.optimizer}'
    else:
        experiment_name = f'{args.model}_{args.dataset}_lr{args.lr}_batch{args.batch}_epoch{args.epoch}_LabelNoiseRate{args.label_noise_rate}_Optim{args.optimizer}'
    ...
    #get initial values
    train_loss, train_combined_acc, train_digit_acc, train_color_acc = test_model(model, train_loader, criterion, device, num_digits, num_colors)
    test_loss, test_combined_acc, test_digit_acc, test_color_acc = test_model(model, test_loader, criterion, device, num_digits, num_colors)
    os.makedirs(f"./csv/combine/{experiment_name}", exist_ok=True)
    with open(f"./csv/combine/{experiment_name}/log.csv", 'a') as f:
        writer = csv.writer(f)
        f.write("epoch, train_loss, train_combined_acc, train_digit_acc, train_color_acc, test_loss, test_combined_acc, test_digit_acc, test_color_acc\n")
        f.write(f"0, {train_loss}, {train_combined_acc}, {train_digit_acc}, {train_color_acc}, {test_loss}, {test_combined_acc}, {test_digit_acc}, {test_color_acc}\n")
    
    # train model
    for epoch in range(args.epoch):
        epoch = epoch + 1 # start from 1
        train_loss, train_combined_acc, train_digit_acc, train_color_acc = train_model(model, train_loader, optimizer, criterion, device, num_digits, num_colors)
        test_loss, test_combined_acc, test_digit_acc, test_color_acc = test_model(model, test_loader, criterion, device, num_digits, num_colors)
        print(f"epoch: {epoch}, train_loss: {train_loss:.4f}, train_combined_acc: {train_combined_acc:.4f}, train_digit_acc: {train_digit_acc:.4f}, train_color_acc: {train_color_acc:.4f}, test_loss: {test_loss:.4f}, test_combined_acc: {test_combined_acc:.4f}, test_digit_acc: {test_digit_acc:.4f}, test_color_acc: {test_color_acc:.4f}")
        if args.wandb:
            wandb.log({"epoch": epoch, "train_loss": train_loss, "train_combined_acc": train_combined_acc, "train_digit_acc": train_digit_acc, "train_color_acc": train_color_acc, "test_loss": test_loss, "test_combined_acc": test_combined_acc, "test_digit_acc": test_digit_acc, "test_color_acc": test_color_acc})
        
        #save logs to ./csv/
        with open(f"./csv/combine/{experiment_name}/log.csv", 'a') as f:
            writer = csv.writer(f)
            f.write(f"{epoch}, {train_loss}, {train_combined_acc}, {train_digit_acc}, {train_color_acc}, {test_loss}, {test_combined_acc}, {test_digit_acc}, {test_color_acc}\n")
