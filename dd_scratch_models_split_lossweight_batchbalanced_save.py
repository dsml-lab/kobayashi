...
def parse_args():
    ...
    #set model settings
    arg_parser.add_argument("--model", type=str, choices=["cnn_2layers", "cnn_3layers", "cnn_4layers", "cnn_5layers", "cnn_8layers", "cnn_16layers", "resnet18", "vgg16"], help="モデルアーキテクチャの選択")
...
def load_models(in_channels, args, img_size, num_classes):
    if args.model == "cnn_2layers":
        ...
        model = VGG16(in_channels, num_classes, args.model_width, img_size)
    else:
    ...
    try:
        ...
        for epoch in range(1, args.epoch + 1):
            try:
                ...
                save_dict = {
                    "args": vars(args),                         # どのような引数で実行したか
                    "epoch": epoch,                             # 現在のエポック数
                    "model_state_dict": model.state_dict(),     # モデルの重み
                    "optimizer_state_dict": optimizer.state_dict(),  # (再学習するなら) オプティマイザの状態
                    "train_accuracy": train_metrics['accuracy_total'],
                    "test_accuracy": test_metrics['accuracy_total']
                }
                # 保存用ディレクトリ作成
                os.makedirs(f"save_model/{experiment_name}", exist_ok=True)
                # 実際に保存
                torch.save(save_dict, f"save_model/{experiment_name}/model_epoch_{epoch}.pth")                
