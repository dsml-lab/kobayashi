...
def parse_args():
    ...
    #set dataset setting
    arg_parser.add_argument("-datasets", "--dataset", type=str, choices=["mnist", "emnist", "emnist_digits", "cifar10", "cifar100", "tinyImageNet", "colored_emnist", "distribution_colored_emnist", "small_emnist_digits"], default="cifar10")
...
# クラスごとにサンプル数を制限する
def reduce_class_samples(x, y, sample_fraction=0.01):
    ...
    for cls in unique_classes:
        ...
        sample_size = max(1, int(len(cls_indices) * sample_fraction))
    ...
    return np.concatenate(reduced_x), np.concatenate(reduced_y)
...
def load_datasets(dataset, target, gray_scale, args):
    ...
    if dataset == "mnist":
    ...
    elif dataset == "small_emnist_digits":
        emnist_path = './data/EMNIST'
        def load_gz_file(file_path, is_image=True):
            with gzip.open(file_path, 'rb') as f:
                if is_image:
        ...
        x_train, y_train = reduce_class_samples(x_train, y_train, sample_fraction=0.01)
        x_test, y_test = reduce_class_samples(x_test, y_test, sample_fraction=0.01)
        # 変換関数が必要な場合はここで定義
        transform = transforms.Compose([
            transforms.ToPILImage(),  # Convert numpy array to PIL Image
            transforms.Resize((32, 32)),  # Same size as original, adjust if needed
...
def main():
    ...
    if args.label_noise_rate > 0.0:
        if args.dataset in ["colored_emnist", "distribution_colored_emnist", "emnist_digits", "small_emnist_digits"]:
    ...
    # set experiment name
    experiment_name = f'{args.model}_{args.dataset}_{args.target}_lr{args.lr}_batch{args.batch_size}_epoch{args.epoch}_LabelNoiseRate{args.label_noise_rate}_Optim{args.optimizer}_Seed{args.fix_seed}'
