...
def train_model(model, train_loader, optimizer, criterion_noisy, criterion_clean, weight_noisy, weight_clean, device, num_digits, num_colors):
    model.train()
    running_loss = 0.0
    total_samples = 0
    correct_total = 0
    ...
    # Initialize counters for digit and color accuracies
    ...
    for inputs, labels, noise_labels in train_loader:
        ...
        # Extract digit and color labels
        predicted_digits = predicted // num_colors
        predicted_colors = predicted % num_colors
        labels_digits = labels // num_colors
        labels_colors = labels % num_colors

        # Compute digit and color accuracies
        correct_digit_total += (predicted_digits == labels_digits).sum().item()
        correct_color_total += (predicted_colors == labels_colors).sum().item()

        # Separate noisy and clean samples
        ...
        if idx_noisy.sum() > 0:
            ...
            running_loss_noisy += loss_noisy.item() * idx_noisy.sum().item()
            loss += loss_noisy * weight_noisy

            correct_noisy += (predicted[idx_noisy] == labels_noisy).sum().item()
            total_noisy += idx_noisy.sum().item()

            # Compute digit and color accuracies for noisy samples
            correct_digit_noisy += (predicted_digits[idx_noisy] == labels_digits[idx_noisy]).sum().item()
            correct_color_noisy += (predicted_colors[idx_noisy] == labels_colors[idx_noisy]).sum().item()

        if idx_clean.sum() > 0:
            outputs_clean = outputs[idx_clean]
            labels_clean = labels[idx_clean]
            loss_clean = criterion_clean(outputs_clean, labels_clean)
            running_loss_clean += loss_clean.item() * idx_clean.sum().item()
            loss += loss_clean * weight_clean

            correct_clean += (predicted[idx_clean] == labels_clean).sum().item()
            total_clean += idx_clean.sum().item()

            # Compute digit and color accuracies for clean samples
            correct_digit_clean += (predicted_digits[idx_clean] == labels_digits[idx_clean]).sum().item()
            correct_color_clean += (predicted_colors[idx_clean] == labels_colors[idx_clean]).sum().item()
    ...
    # Compute digit and color accuracies
    digit_accuracy_total = 100. * correct_digit_total / total_samples
    color_accuracy_total = 100. * correct_color_total / total_samples

    digit_accuracy_noisy = 100. * correct_digit_noisy / total_noisy if total_noisy > 0 else float('nan')
    color_accuracy_noisy = 100. * correct_color_noisy / total_noisy if total_noisy > 0 else float('nan')

    digit_accuracy_clean = 100. * correct_digit_clean / total_clean if total_clean > 0 else float('nan')
    color_accuracy_clean = 100. * correct_color_clean / total_clean if total_clean > 0 else float('nan')
    ...
    return avg_loss, accuracy_total, accuracy_noisy, accuracy_clean, avg_loss_noisy, avg_loss_clean, \
        ...
        digit_accuracy_total, color_accuracy_total, digit_accuracy_noisy, color_accuracy_noisy, \
        digit_accuracy_clean, color_accuracy_clean
...
def test_model(model, test_loader, criterion, device, num_digits, num_colors):
    ...
    with torch.no_grad():
        for inputs, labels in test_loader:  # Note: test_loader does not have noise_info
            ...
            correct_digit += (predicted_digits == labels_digits).sum().item()
            correct_color += (predicted_colors == labels_colors).sum().item()

    avg_loss = test_loss / total
    accuracy = 100. * correct / total
    digit_accuracy = 100. * correct_digit / total
    color_accuracy = 100. * correct_color / total

    return avg_loss, accuracy, total, correct, digit_accuracy, color_accuracy
...
def main():
    ...
    for epoch in range(1, args.epoch + 1):
        ...
        test_loss, test_accuracy, test_total_samples, test_correct = test_model(model, test_loader, criterion_clean, device)
