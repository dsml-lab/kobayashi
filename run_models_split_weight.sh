...
NUM_WORKERS=4
WANDB=true
WANDB_PROJECT="dd_distribution_color_emnist_split_lossweight_all_v2"
WANDB_ENTITY="dsml-kernel24"
MODEL="cnn_5layers"

# パラメータの組み合わせ
LABEL_NOISE_RATES=(0.5)
WEIGHT_NOISY_LIST=(0.0 0.01 0.1 0.5 0.7 1.0)
...
for LABEL_NOISE_RATE in "${LABEL_NOISE_RATES[@]}"; do
  for WEIGHT_NOISY in "${WEIGHT_NOISY_LIST[@]}"; do
    for WEIGHT_CLEAN in "${WEIGHT_CLEAN_LIST[@]}"; do
      cmd="python dd_scratch_models_split_lossweight.py \
        --fix_seed $SEED \
        --model $MODEL \
