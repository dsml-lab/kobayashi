...
def parse_args():
    ...
    arg_parser.add_argument("--smoothing", type=float, default=0.1, help="ラベルスムージングの割合")
...
def main():
    ...
    train_criterion = nn.CrossEntropyLoss(reduction='none',label_smoothing=args.smoothing)
    weight_noisy = args.weight_noisy
    weight_clean = args.weight_clean

    # Set experiment name
    experiment_name = f'label_smoothing{args.smoothing}_{args.model}_{args.dataset}_{args.target}_lr{args.lr}_batch{args.batch_size}_epoch{args.epoch}_LabelNoiseRate{args.label_noise_rate}_Optim{args.optimizer}_cleanw{args.weight_clean}_noisew{args.weight_noisy}'
    ...
    for epoch in range(1, args.epoch + 1):
        train_metrics = train_model(
            model, train_loader, optimizer, train_criterion, weight_noisy, weight_clean, device, num_colors, num_digits
        )
