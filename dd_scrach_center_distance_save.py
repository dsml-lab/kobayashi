...
from logger import (
    setup_wandb,
    setup_alpha_csv_logging,
    log_alpha_test_results,
    log_to_wandb,
    setup_alpha_csv_logging_save,
    log_alpha_test_results_save,
    setup_alpha_csv_logging_save_dir
)
...
def main():
    ...
    try:
        ...
        # きれいなデータセットのラベルを original_targets が持っている。
        ...
        # 画像保存用ディレクトリは fig_and_log_dir を使用
        ...
        axes[0].set_title(
            f"Class {n2} Data (idx={idx2})\nOri: {orig_label_x2},N: {noisy_label_x2},C_Dis={distance2:.2f}"
        )
        axes[0].axis('off')
        
        axes[1].imshow(x1_np.squeeze(), cmap=cmap)
        axes[1].set_title(
            f"Class {n1} Data (idx={idx1})\nOri: {orig_label_x1},N: {noisy_label_x1},C_Dis={distance1:.2f},Dis_x1_x2={distance_x1_x2:.2f}"
        )
        axes[1].axis('off')
        ...
        for epoch in range(0, args.epoch + 1):
            ...
            # ここから CSV を csv_dir に保存するように修正
            # 元の関数呼び出しはそのまま活かしつつ、返り値のファイル名を csv_dir に連結
    ...
    #target_plot_probabilities(alpha_csv_dir,targets="color",gif=True,gif_output=f"color_gif.gif",epoch_start=0,epoch_end=2000,epoch_step=5)
    #target_plot_probabilities(alpha_csv_dir,targets="digit",gif=True,gif_output=f"digit_gif.gif",epoch_start=0,epoch_end=2000,epoch_step=5)
    clear_memory()
